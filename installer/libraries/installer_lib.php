...
class Installer_lib {
	...
	public function install($data)
	{
		...
		include '../application/config/migration.php';

		// Get the SQL for the default data and parse it
		//$user_sql = file_get_contents('./sql/default.sql');
		$user_sql = file_get_contents('./sql/release.sql');
		//$user_sql = str_replace('{PREFIX}', $data['site_ref'].'_', $user_sql);
		//$user_sql = str_replace('{EMAIL}', $data['user_email'], $user_sql);
		$user_sql = str_replace('{GROUP-ID}', 1000, $user_sql);
		$user_sql = str_replace('{USER-NAME}', mysql_escape_string($data['user_name']), $user_sql);
		$user_sql = str_replace('{FNAME}', mysql_escape_string($data['user_firstname']), $user_sql);
		$user_sql = str_replace('{LNAME}', mysql_escape_string($data['user_lastname']), $user_sql);
		$user_sql = str_replace('{MNAME}', '', $user_sql);
